...
- (void)commonInit {
    ...
    self.scrollView.accessibilityLabel = @"HMSegmentedControl scrollView";
    ...
    self.selectedSegmentIndex = HMSegmentedControlNoSegment;
    ...
    if (self.selectedSegmentIndex != HMSegmentedControlNoSegment) {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
            if (!self.selectionIndicatorArrowLayer.superlayer) {
                [self setArrowFrameWithIndex:self.selectedSegmentIndex];
                [self.scrollView.layer addSublayer:self.selectionIndicatorArrowLayer];
                if (self.firstDraw) {
                    self.firstDraw = NO;
                    CGRect scrollRect = self.selectionIndicatorArrowLayer.frame;
                    CGFloat selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (CGRectGetWidth(scrollRect) / 2);
                    scrollRect.origin.x -= selectedSegmentOffset;
                    scrollRect.size.width = CGRectGetWidth(self.frame);
                    [self.scrollView scrollRectToVisible:scrollRect animated:NO];
                }
            }
        } else {
            if (!self.selectionIndicatorStripLayer.superlayer) {
                self.selectionIndicatorStripLayer.frame = [self frameForSelectionIndicatorWithIndex:self.selectedSegmentIndex];
                [self.scrollView.layer addSublayer:self.selectionIndicatorStripLayer];
                if (self.firstDraw) {
                    self.firstDraw = NO;
                    CGRect scrollRect = self.selectionIndicatorStripLayer.frame;
                    CGFloat selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (CGRectGetWidth(scrollRect) / 2);
                    scrollRect.origin.x -= selectedSegmentOffset;
                    scrollRect.size.width = CGRectGetWidth(self.frame);
                    [self.scrollView scrollRectToVisible:scrollRect animated:NO];
                ...
                if (self.selectionStyle == HMSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
                    self.selectionIndicatorBoxLayer.frame = [self frameForFillerSelectionIndicatorWithIndex:self.selectedSegmentIndex];
...
- (void)setArrowFrameWithIndex:(NSInteger)segmentIndex {
    CGRect rect = [self frameForSelectionIndicatorWithIndex:segmentIndex];
    if (rect.size.width != self.selectionIndicatorArrowLayer.frame.size.width || rect.size.height != self.selectionIndicatorArrowLayer.frame.size.height) {
        self.selectionIndicatorArrowLayer.frame = rect;
...
//                    p3 = CGPointMake(self.selectionIndicatorArrowLayer.bounds.size.width, self.selectionIndicatorArrowLayer.bounds.size.height);
            p1 = CGPointMake(0, 0);
            p2 = CGPointMake(self.selectionIndicatorArrowLayer.bounds.size.width / 2, self.selectionIndicatorArrowLayer.bounds.size.height);
            p3 = CGPointMake(self.selectionIndicatorArrowLayer.bounds.size.width, 0);
        }
...
- (CGRect)frameForSelectionIndicatorWithIndex:(NSInteger)segmentIndex {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        CGFloat stringWidth = [self measureTitleAtIndex:segmentIndex].width;
        sectionWidth = stringWidth;
    } else if (self.type == HMSegmentedControlTypeImages) {
        UIImage *sectionImage = [self.sectionImages objectAtIndex:segmentIndex];
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * segmentIndex);
        CGFloat widthToEndOfSelectedSegment = widthToStartOfSelectedIndex + self.segmentWidth;
        ...
        if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
            ...
            widthToStartOfSelectedIndex = selectedSegmentOffset;
            widthToEndOfSelectedSegment = widthToStartOfSelectedIndex + self.segmentWidthsArray[segmentIndex].floatValue;
        }
        
        CGFloat x = widthToStartOfSelectedIndex + ((widthToEndOfSelectedSegment - widthToStartOfSelectedIndex) / 2);
        return CGRectMake(x - self.selectionIndicatorHeight / tanf(M_PI / 3.0), indicatorYOffset, floorf(self.selectionIndicatorHeight / tanf(M_PI / 3.0) * 2), self.selectionIndicatorHeight);
    } else {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * segmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * segmentIndex);
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                ...
                return CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, [[self.segmentWidthsArray objectAtIndex:segmentIndex] floatValue] - self.selectionIndicatorEdgeInsets.right - self.selectionIndicatorEdgeInsets.left, self.selectionIndicatorHeight);
            }
            
            return CGRectMake((self.segmentWidth + self.selectionIndicatorEdgeInsets.left) * segmentIndex, indicatorYOffset, self.segmentWidth - self.selectionIndicatorEdgeInsets.right - self.selectionIndicatorEdgeInsets.left, self.selectionIndicatorHeight);
        }
    }
}

- (CGRect)frameForFillerSelectionIndicatorWithIndex:(NSInteger)segmentIndex {
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
        CGFloat selectedSegmentOffset = 0.0f;
        ...
        return CGRectMake(selectedSegmentOffset, 0, [[self.segmentWidthsArray objectAtIndex:segmentIndex] floatValue], CGRectGetHeight(self.frame));
    }
    return CGRectMake(self.segmentWidth * segmentIndex, 0, self.segmentWidth, CGRectGetHeight(self.frame));
}
...
- (void)scrollFrom:(NSInteger)fromIndex to:(NSInteger)toIndex with:(CGFloat)progress {
    if (fromIndex != HMSegmentedControlNoSegment
        && 0 <= fromIndex && fromIndex < self.sectionTitles.count
        && 0 <= toIndex && toIndex < self.sectionTitles.count) {
        // Disable CALayer animations
        NSMutableDictionary *newActions = [[NSMutableDictionary alloc] initWithObjectsAndKeys:[NSNull null], @"position", [NSNull null], @"bounds", nil];
        self.selectionIndicatorArrowLayer.actions = newActions;
        
        self.selectionIndicatorStripLayer.actions = newActions;
        self.selectionIndicatorStripLayer.frame = [self frameForSelectionIndicatorFrom:fromIndex to:toIndex with:progress];
        
        self.selectionIndicatorBoxLayer.actions = newActions;
        self.selectionIndicatorBoxLayer.frame = [self frameForFillerSelectionIndicatorFrom:fromIndex to:toIndex with:progress];
        
        self.selectionIndicatorArrowLayer.frame = self.selectionIndicatorStripLayer.frame;
        [self layoutIfNeeded];
    }
}

- (CGRect)frameForSelectionIndicatorFrom:(NSInteger)fromIndex to:(NSInteger)toIndex with:(CGFloat)progress {
    CGRect fromRect = [self frameForSelectionIndicatorWithIndex:fromIndex];
    CGRect toRect = [self frameForSelectionIndicatorWithIndex:toIndex];
    CGFloat x = fromRect.origin.x + progress * (toRect.origin.x - fromRect.origin.x);
    CGFloat y = fromRect.origin.y;
    CGFloat width = fromRect.size.width + progress * (toRect.size.width - fromRect.size.width);
    CGFloat height = fromRect.size.height;
    return CGRectMake(x, y, width, height);
}

- (CGRect)frameForFillerSelectionIndicatorFrom:(NSInteger)fromIndex to:(NSInteger)toIndex with:(CGFloat)progress {
    CGRect fromRect = [self frameForFillerSelectionIndicatorWithIndex:fromIndex];
    CGRect toRect = [self frameForFillerSelectionIndicatorWithIndex:toIndex];
    CGFloat x = fromRect.origin.x + progress * (toRect.origin.x - fromRect.origin.x);
    CGFloat y = fromRect.origin.y;
    CGFloat width = fromRect.size.width + progress * (toRect.size.width - fromRect.size.width);
    CGFloat height = fromRect.size.height;
